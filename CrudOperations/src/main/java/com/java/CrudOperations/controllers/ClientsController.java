...
import com.java.CrudOperations.models.ClientDto;
import com.java.CrudOperations.repositories.ClientRepository;
...
public class ClientsController {
    ...
    public String createClient(
        ...
        if (clientRepo.findByEmail(clientDto.getEmail()) != null) {
            result.addError(
                    new FieldError(
                            "clientDto", "email", clientDto.getEmail(), false, null, null, "Email address is already used")
            );
        }
    ...
    public String editClient( Model model, @RequestParam int id , @Valid @ModelAttribute ClientDto clientDto, BindingResult result) {
    	...
    	try {
    		// Here in model we said email should be unique so since it will be getting an error if it is duplicate
    		clientRepo.save(client);
    	}
    	catch(Exception ex) {
